// Package services manages the catalogue of download services: the
// cache-domains lists (uklans/cache-domains) as source, enable/disable
// state, custom services and hosts, the anchored host matchers used by
// DNS/proxy/SNI, the special pass-through path rules, and content
// grouping/labels (docs/ARCHITECTURE.md 8.1, 8.2 steps 5–6, 8.4).
//
...
// downloadCache.disabledServices changes and refetches when domainsSource changes.
package services
...
// SteamTrigger is the hostname Steam uses to discover a download cache.
...
// Service is one download service.
type Service struct {
...
func (r *Registry) onSettings(old, n *settings.All) {
	if !slices.Equal(old.DownloadCache.DisabledServices, n.DownloadCache.DisabledServices) {
		r.mu.Lock()
		r.rebuildLocked(n)
		r.mu.Unlock()
	}
	if old.DownloadCache.DomainsSource != n.DownloadCache.DomainsSource {
		signal(r.refetch)
	}
	if old.DownloadCache.UpdateIntervalHours != n.DownloadCache.UpdateIntervalHours {
		signal(r.resched)
	}
}
...
// downloadCache.updateIntervalHours (±10 %, 0 = manual only) and refetches when
...
func (r *Registry) Refresh(ctx context.Context) error {
	...
	base := r.set.Get().DownloadCache.DomainsSource
...
func (r *Registry) Status() SourceStatus {
	...
	r.mu.Unlock()
	st.Source = redactURL(r.set.Get().DownloadCache.DomainsSource)
	return st
}
