...
const (
	// processedByHeader is the response header prefill tools check; it also
	// carries the instance ID for loop detection.
...
// X-Forwarded-For) and the processed-by header is replaced by ours.
var neverForwarded = map[string]bool{
	...
	"Forwarded":               true,
	"X-Real-Ip":               true,
	"X-Lancache-Processed-By": true, // processedByHeader, canonical form
}
...
// storedDeniedPrefixes are lower-case name prefixes never recorded (cache
// status headers of CDNs and other caches, including the processed-by header
// family).
...
// end-to-end headers only, plus the processed-by header. For fills and
