...
func Defaults() All {
	return All{
		DNS: DNS{
			// Quad9 filters malware; the fallback is another operator that
			// filters malware too (its blocks are detected like Quad9's).
			Upstreams:         []string{"https://dns.quad9.net/dns-query"},
			FallbackUpstreams: []string{"https://security.cloudflare-dns.com/dns-query"},
			Bootstrap:         []string{"9.9.9.9", "149.112.112.112", "1.1.1.1", "1.0.0.1", "2620:fe::fe", "2606:4700:4700::1111"},
			UpstreamMode:      "load_balance",
			UpstreamTimeoutMs: 10000,
			// UpstreamBlockedTTL: a blocked answer is re-checked after 5 minutes.
			UpstreamBlockedTTL:     300,
			ECS:                    ECS{Mode: ECSOff},
			...
			RateLimitIPv6Prefix:    64,
			RateLimitExempt:        []string{},
			RefuseANY:              true,
			EDNSClientTrusted:      []string{},
			RebindProtection:       true,
			RebindAllow:            []string{"plex.direct"}, // Plex answers its names with LAN addresses on purpose
			DomainNeeded:           true,
