...
// unix milliseconds like everywhere in picache.db.
...
func (e *Engine) load(ctx context.Context, id int64) ([]row, error) {
	...
	rows, err := e.db.R.QueryContext(ctx, `SELECT group_id, config, override_mode, COALESCE(override_until, 0), updated_at
		FROM parental_groups`+where+` ORDER BY group_id`, args...)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.groupID, &cfg, &mode, &until, &updated); err != nil {
			return nil, fmt.Errorf("parental: load: %w", err)
		}
		if err := json.Unmarshal([]byte(cfg), &r.cfg); err != nil {
			e.log.Warn("unreadable parental controls ignored", slog.Int64("group", r.groupID), slog.Any("err", err))
			r.cfg = Config{}
		}
		r.cfg = sanitizeConfig(r.cfg)
...
// unknown group), removes expired overrides of all groups and reloads the
// snapshot.
func (e *Engine) write(ctx context.Context, id int64, fn func(writer) error) error {
	...
	err := e.db.Tx(ctx, func(tx *sql.Tx) error {
		...
		return fn(writer{ctx: ctx, tx: tx, id: id, now: now})
	})
	if err != nil {
		return err
	}
...
// validServices checks a list of service ids (at least min, at most 64,
...
// sanitizeConfig keeps the valid parts of a stored configuration.
func sanitizeConfig(c Config) Config {
	out := Config{BlockedServices: knownServices(c.BlockedServices), Schedules: []Schedule{}}
