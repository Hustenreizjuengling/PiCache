...
func TestMatchDNS(t *testing.T) {
	...
	tests := []struct {
		...
		{SteamTrigger, "steam"},
		{"trigger.lancache.net", ""}, // service "test" is disabled by default
		{"example.com", ""},
		{"", ""},
		{".", ""},
	}
...
func TestClassify(t *testing.T) {
	...
	tests := []struct {
		...
		{"disabled service is known", "trigger.lancache.net", "", "/", "test", false, true},
...
func TestDisabledServicesRebuild(t *testing.T) {
	...
	if !slices.Contains(e.set.Get().LanCache.DisabledServices, "blizzard") {
	...
	if _, err := e.set.Update(ctx, func(a *settings.All) error { a.LanCache.DisabledServices = nil; return nil }); err != nil {
		t.Fatal(err)
	}
	if _, ok := r.MatchDNS("level3.blizzard.com"); !ok {
		t.Fatal("re-enabled service not matched")
	}
	if id, ok := r.MatchDNS("trigger.lancache.net"); !ok || id != "test" {
