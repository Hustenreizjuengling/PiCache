...
// setDisabled adds id to or removes it from lancache.disabledServices; the
...
func (r *Registry) setDisabled(ctx context.Context, id string, disabled bool) error {
	_, err := r.set.Update(ctx, func(a *settings.All) error {
		list := slices.DeleteFunc(slices.Clone(a.LanCache.DisabledServices), func(s string) bool { return s == id })
		if disabled {
			list = append(list, id)
		}
		a.LanCache.DisabledServices = list
		return nil
	})
	return err
}
...
func (r *Registry) DeleteCustom(ctx context.Context, id string) error {
	...
	if slices.Contains(r.set.Get().LanCache.DisabledServices, id) {
