...
func (s *Server) handle(ctx context.Context, c net.Conn) {
	...
	if s.d.Settings != nil && !s.d.Settings.Get().LanCache.Enabled {
		s.refuse(ip, name, "LanCache is disabled")
		return
	}
	service, ok := "", false
	if s.d.Services != nil {
...
// link-local; private addresses only with lancache.allowPrivateUpstreams.
func (s *Server) dialUpstream(ctx context.Context, host string) (net.Conn, error) {
	...
	d := netutil.SafeDialer{
		...
		AllowPrivate: func(context.Context) bool {
			return s.d.Settings != nil && s.d.Settings.Get().LanCache.AllowPrivateUpstreams
		},
		Timeout:  dialTimeout,
		OwnAddrs: s.ownAddrs,
	}
