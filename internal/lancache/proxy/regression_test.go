...
// TestLanCacheDisabled: with LanCache off (the default) the cache proxy
// answers only the heartbeat; nothing is fetched or stored.
func TestLanCacheDisabled(t *testing.T) {
	h := newHarness(t)
	h.origin.set("/f", &originObj{data: testData(3000)})
	h.updateSettings(func(a *settings.All) { a.LanCache.Enabled = false })
	...
	if n := len(h.origin.requests()); n != 0 {
		t.Fatalf("%d upstream requests while LanCache is disabled", n)
	}
	if h.store.object(testService, "/f") != nil || h.store.object("steam", "/depot/7/chunk/x") != nil {
		t.Fatal("content stored while LanCache is disabled")
	}
...
func TestCollapsingWithoutRanges(t *testing.T) {
	...
	for _, marked := range []bool{false, true} {
		...
		if got := len(h.origin.ranges("/whole")); got != 1 {
