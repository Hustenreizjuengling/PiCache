...
func New(ctx context.Context, d Deps) (*Server, error) {
	...
	s.safe = &netutil.SafeDialer{
		Resolve:      d.Lookup,
		AllowPrivate: func(context.Context) bool { return s.settings().LanCache.AllowPrivateUpstreams },
...
// nocacheSet caches the parsed lancache.nocacheClients of one settings snapshot.
...
func (s *Server) nocacheAllowed(ip netip.Addr) bool {
	...
	if ns == nil || ns.src != all {
		ns = &nocacheSet{src: all, prefixes: settings.ParsePrefixes(all.LanCache.NocacheClients)}
		s.nocache.Store(ns)
	}
	ip = netutil.Canon(ip)
