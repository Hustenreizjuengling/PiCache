...
func (s *Store) Summary(ctx context.Context, from, to time.Time) (Summary, error) {
	...
	sum := Summary{From: from, To: to, TopFrom: TopFrom(from, to), DroppedLogEvents: s.dropped.Load(), UniqueDomainsEstimated: true}
	...
	sp := spanFor(from, to, hour)
	if !sp.mem {
		dnsClients, cacheClients, uniq = nil, nil, hll{}
	}
	...
	if err := s.d.R.QueryRowContext(ctx, spanCTE+`SELECT COUNT(*) FROM (
			SELECT t.key FROM days CROSS JOIN logs_dns_top_daily t WHERE t.bucket = days.b AND t.kind = 'client'
			UNION SELECT t.key FROM hours CROSS JOIN logs_dns_top_hourly t WHERE t.bucket = hours.b AND t.kind = 'client'
			UNION SELECT t.key FROM days CROSS JOIN logs_cache_top_daily t WHERE t.bucket = days.b AND t.kind = 'client'
			UNION SELECT t.key FROM hours CROSS JOIN logs_cache_top_hourly t WHERE t.bucket = hours.b AND t.kind = 'client'
			UNION SELECT json_extract(value, '$.k') FROM json_each(?5)
			UNION SELECT json_extract(value, '$.k') FROM json_each(?6))`,
		append(sp.args(), string(dnsJSON), string(cacheJSON))...).Scan(&sum.ActiveClients); err != nil {
		return Summary{}, queryErr(ctx, err)
	}
	if sum.UniqueDomains, err = s.uniqueDomains(ctx, sp, &uniq); err != nil {
		return Summary{}, queryErr(ctx, err)
	}
...
// uniqueDomains merges the unique sketches of a span with mem (the
// in-memory hour's, empty when it is not in the span) and returns the
// estimate. The caller holds a query slot.
func (s *Store) uniqueDomains(ctx context.Context, sp topSpan, mem *hll) (int64, error) {
	rows, err := s.d.R.QueryContext(ctx, spanCTE+`
		SELECT t.label FROM days CROSS JOIN logs_dns_top_daily t WHERE t.bucket = days.b AND t.kind = ?5 AND t.key = ''
		UNION ALL
		SELECT t.label FROM hours CROSS JOIN logs_dns_top_hourly t WHERE t.bucket = hours.b AND t.kind = ?5 AND t.key = ''`,
		append(sp.args(), uniqueKind)...)
	if err != nil {
		return 0, err
	}
	defer rows.Close()
...
// Top returns a top list for [TopFrom(from, to), to).
func (s *Store) Top(ctx context.Context, kind TopKind, from, to time.Time, limit int) ([]TopItem, error) {
	...
	case TopDomains, TopBlockedDomains, TopClients, TopUpstreams:
		...
		for _, r := range rows {
			...
			case dnsTopUpstream:
				if r.Count > 0 {
					it.AvgDurationUs = r.DurationUs / r.Count
					it.Bytes = it.AvgDurationUs // the member of 0.11 and before
				}
			}
			out = append(out, it)
		}
...
// to), to) by purpose, from the top tables and the in-memory hour.
func (s *Store) Purposes(ctx context.Context, from, to time.Time) (PurposeStats, error) {
	...
	out := PurposeStats{From: TopFrom(from, to), Purposes: make([]PurposeCount, 0, len(rows))}
	for _, r := range rows {
...
func (s *Store) QTypes(ctx context.Context, from, to time.Time) (QTypeStats, error) {
	from, to, err := statsRange(from, to)
	if err != nil {
		return QTypeStats{}, err
	}
	rows, err := s.dnsTop(ctx, dnsTopQType, from, to, topKeysPerHour)
	if err != nil {
		return QTypeStats{}, err
	}
	out := QTypeStats{From: TopFrom(from, to), QTypes: make([]QTypeCount, 0, min(len(rows), maxQTypeKeys+1))}
	var other int64
	for _, r := range rows { // most first
		if r.Key == qtypeOther || len(out.QTypes) == maxQTypeKeys {
			other += r.Count
			continue
		}
		out.QTypes = append(out.QTypes, QTypeCount{QType: r.Key, Count: r.Count})
	}
	if other > 0 {
		out.QTypes = append(out.QTypes, QTypeCount{QType: qtypeOther, Count: other})
		slices.SortStableFunc(out.QTypes, func(a, b QTypeCount) int {
			return cmp.Or(cmp.Compare(b.Count, a.Count), cmp.Compare(a.QType, b.QType))
		})
	}
	return out, nil
}

// dnsTop aggregates the DNS top rows of kind k over [TopFrom(from, to), to)
// (the hourly rows, for ranges longer than 7 days the daily rows of the
// complete days and the hourly rows of the last day), merged with the
// in-memory current hour, highest count first.
func (s *Store) dnsTop(ctx context.Context, k dnsKind, from, to time.Time, limit int) ([]dnsTopRow, error) {
	...
	rows, err := s.d.R.QueryContext(ctx, spanCTE+`SELECT key, label, SUM(count) AS c, SUM(blocked), SUM(duration_us), MAX(last_seen)
		FROM (
			SELECT t.key, t.label, t.count, t.blocked, t.duration_us, t.last_seen
			FROM days CROSS JOIN logs_dns_top_daily t WHERE t.bucket = days.b AND t.kind = ?5
			UNION ALL
			SELECT t.key, t.label, t.count, t.blocked, t.duration_us, t.last_seen
			FROM hours CROSS JOIN logs_dns_top_hourly t WHERE t.bucket = hours.b AND t.kind = ?5
			UNION ALL
			...
			FROM json_each(?6)
		) GROUP BY key ORDER BY c DESC, key LIMIT ?7`, append(sp.args(), dnsKindNames[k], string(memJSON), limit)...)
	if err != nil {
...
// cacheTop aggregates the cache top rows of kind k over [TopFrom(from,
// to), to) like dnsTop, merged with the in-memory current hour, most bytes
// sent first.
func (s *Store) cacheTop(ctx context.Context, k cacheKind, from, to time.Time, limit int) ([]cacheTopRow, error) {
	...
	defer release()
	rows, err := s.d.R.QueryContext(ctx, spanCTE+`SELECT service, key, label, SUM(requests), SUM(bytes_sent) AS b, SUM(bytes_hit),
			SUM(bytes_wan), MAX(last_seen)
		FROM (
			SELECT t.service, t.key, t.label, t.requests, t.bytes_sent, t.bytes_hit, t.bytes_wan, t.last_seen
			FROM days CROSS JOIN logs_cache_top_daily t WHERE t.bucket = days.b AND t.kind = ?5
			UNION ALL
			SELECT t.service, t.key, t.label, t.requests, t.bytes_sent, t.bytes_hit, t.bytes_wan, t.last_seen
			FROM hours CROSS JOIN logs_cache_top_hourly t WHERE t.bucket = hours.b AND t.kind = ?5
			UNION ALL
			...
			FROM json_each(?6)
		) GROUP BY service, key ORDER BY b DESC, service, key LIMIT ?7`,
		append(sp.args(), cacheKindNames[k], string(memJSON), limit)...)
	if err != nil {
		return nil, queryErr(ctx, err)
	}
	defer rows.Close()
...
// ClientStats aggregates per client for [TopFrom(from, to), to) (at most
// 1000 clients, most queries first).
