...
import { api, resource, type LanCacheService } from '$lib/api'
import type { SelectOption } from '$lib/ui'

export interface ServiceCatalog {
  /** Services as returned by /lancache/services (domains trimmed). */
  readonly list: readonly LanCacheService[]
...
export function serviceCatalog(): ServiceCatalog {
  const r = resource((signal) => api.lancache.services({ signal }))
