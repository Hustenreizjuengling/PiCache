...
- Optional: a DHCP server (and IPv6 DNS announcements) for networks whose router cannot hand out another DNS server (18); off by default.
...
| `PICACHE_DHCP` | off | `on`: open the DHCP sockets (UDP 67 and 547, raw ICMPv6 with `CAP_NET_RAW`) for the optional DHCP server (18) | reported in the DHCP status and the health check `dhcp`; never fatal |

All listeners and the DHCP sockets are bound **before** privileges are dropped (section 6.2). TCP listeners for DNS, :80 and :443 are wrapped with `netutil.LimitListener`: connections from clients outside the ACL are closed at accept; concurrent connections are capped per client key (`netutil.ClientKey`: /32 for IPv4; /128 for loopback, link-local, ULA and on-link IPv6; /64 for other IPv6) and in total (DNS 32/1024, cache and SNI 256/4096).
...
| Privilege | Runtime is unprivileged and never holds `CAP_SYS_ADMIN` (6.2). `CAP_NET_RAW` (optional DHCP router advertisements) is used only to open one raw ICMPv6 socket at start and then dropped on every thread, verified (18.1). |
| DHCP (optional, 18) | Off by default and only with `PICACHE_DHCP`. Serves one configured interface only (ingress interface checked per packet), never relayed requests, never with a dynamic own address, never while another DHCP server was detected (probe, passive detection) unless explicitly overridden. Every DHCPv4, DHCPv6 and ICMPv6 packet is untrusted: bounded sizes, every option bounds-checked, malformed packets dropped and counted, a failure while handling one packet drops only that packet. Rate limits (DHCPv4 50/s and 5/s per MAC, DHCPv6 50/s, one answer to router solicitations per 3 s); bounded tables (4096 leases, 1024 static leases, 32 other servers). Router advertisements never make PiCache a router (lifetime 0, no prefixes). Replies go only to the limited broadcast address, to a client address inside the served subnet, or (DHCPv6) to a link-local client, so they cannot be aimed at hosts elsewhere. |
...
- **Bare metal / LXC (systemd)**: user `picache`, `AmbientCapabilities=CAP_NET_BIND_SERVICE`, `NoNewPrivileges=yes`, `ProtectSystem=strict`, full hardening (`deploy/systemd/picache.service`). NAS mounts are done by the optional **root helper**: `picache-storage.path` watches `/var/lib/picache/storage-requests/` and starts `picache-storage.service` (`picache storage apply-pending`, root, oneshot), which re-validates every request (`storage.ValidateTarget`), writes `/etc/picache/credentials/<id>.cred` and a `.mount` unit for `/srv/picache/<id>` and starts it. The main service only writes request files. In unprivileged LXC the helper cannot mount CIFS/NFS (kernel rule); the Proxmox host mounts the share and bind-mounts it (snippets in the UI).
  The optional DHCP server (18) needs `install.sh --with-dhcp`: `PICACHE_DHCP=on` in `picache.env` and the drop-in `picache.service.d/60-dhcp.conf` with `AmbientCapabilities=CAP_NET_BIND_SERVICE CAP_NET_RAW`, `CapabilityBoundingSet=CAP_NET_BIND_SERVICE CAP_NET_RAW` and `SystemCallFilter=capset` (re-allowed after `~@privileged` so the service can drop the capability). `CAP_NET_RAW` is used for the raw ICMPv6 socket at start and then dropped for good (18.1).
...
| DHCP server | off (needs `PICACHE_DHCP`); lease 24 h, register host names on, router = IPv4 default gateway, DNS server = PiCache, domain = local domain; IPv6 router advertisements and DHCPv6 off; `ignoreOtherServers` off |
...
Optional, for routers that cannot hand out another DNS server: PiCache hands out IPv4 addresses (DHCPv4) and announces itself as IPv6 DNS server (router advertisements with RDNSS/DNSSL, stateless DHCPv6). It never serves by surprise: off by default, only with `PICACHE_DHCP`, only on the configured interface, never in the known-dangerous situations (18.2), and it never becomes the IPv6 default router. No DHCPv6 address leases, no prefix delegation, no DHCP relay support.

### 18.1 Privileges and sockets

- With `PICACHE_DHCP=on` (`on`/`1`/`true`) the app opens, at start and before the privilege drop like the listeners: UDP `0.0.0.0:67` (`SO_BROADCAST`, `IP_PKTINFO`: the ingress interface of every packet, and replies leave through the configured interface with PiCache's address as source); UDP `[::]:547` (`IPV6_RECVPKTINFO`); a raw ICMPv6 socket for the router advertisements, only if the process has `CAP_NET_RAW` (hop limit 255, an `ICMP6_FILTER` that passes router solicitations (type 133) only, `IPV6_RECVPKTINFO` and `IPV6_RECVHOPLIMIT`, no multicast loopback). A socket that fails is reported, never fatal: without UDP 67 DHCP is unavailable (health `fail` while enabled; e.g. dnsmasq on the same host), without UDP 547 DHCPv6 is blocked (`no-socket`), without the raw socket the router advertisements are (`no-raw-socket`).
- `CAP_NET_RAW` is dropped for good right after the privilege drop (`app.dropNetRaw`, whether or not DHCP is on): when any thread lists it in `/proc/self/task/*/status` (`CapEff`, `CapPrm`, `CapAmb`), the ambient set is lowered (`PR_CAP_AMBIENT_LOWER`) and the effective, permitted and inheritable sets are set without it on every thread (`syscall.AllThreadsSyscall` with `SYS_CAPSET`; the binary is built without cgo). Then no thread may list it and opening another raw socket must fail. If any of this fails, the raw socket is closed (no router advertisements, the reason in the status) and the error logged; DNS keeps running. systemd grants the capability through the drop-in of `install.sh --with-dhcp` (6.2), which re-allows `capset` after `~@privileged`; in Docker the switch to `PICACHE_RUN_AS` clears every capability and the socket opened before stays usable.
...
- Without `PICACHE_DHCP`, on systems other than Linux and in a container bridge network (DHCP broadcasts do not reach the container) the service is `unavailable`; the tables exist anyway (backups and restores are the same everywhere), the settings and static leases can be edited, and the names of stored leases are answered until they expire.

### 18.2 States and safety gates

State: `unavailable`, `off` (`dhcp.enabled` false), `blocked`, `serving`, `error` (a configuration or runtime problem that is not a blocker: no router address because the IPv4 default gateway is not in the subnet, a failed probe). The gates are evaluated at start, every 30 s and on settings changes; while `enabled` is true PiCache serves only if none applies:
...
- **Messages**: DISCOVER → OFFER (no answer when the pool is exhausted); REQUEST (selecting, init-reboot, renewing, rebinding) → ACK or NAK; a REQUEST naming another server identifier is recorded (18.2) and not answered; DECLINE (of the address the client holds or was offered) → the lease ends and the address is quarantined for 10 minutes; RELEASE → the lease ends (the address stays reserved for the client); INFORM (from an address of the subnet) → ACK with the options, no lease.
- **Authoritative for its subnet**: a REQUEST is nak'ed for an address outside the subnet, a reserved address (network, broadcast, PiCache's, the router's, the DNS server's), the static address of another client, an address with an active lease or a pending offer of another client, a quarantined address, an address kept for another client (storage, below), an address outside the range that is not the client's static address, and a dynamic address while the client has a free static address (so it moves there). An address of the range without any record is acknowledged after the neighbour check (below): a client that got it from the previous DHCP server keeps it.
- **Address choice** for a DISCOVER: the client's static lease; else its current or previous lease; else the requested address (option 50) if free and in the range; else the lowest free address; when the range is exhausted the address whose lease of another client expired first. An offered address is held for its client for 60 s.
...
- **Options** in OFFER and ACK: 1 subnet mask, 3 router (`dhcp.router`, else the IPv4 default gateway, which must be in the subnet; the last one seen is used while the routing table has none), 6 DNS server (`dhcp.dnsServer`, else PiCache's address), 15 domain name and 119 domain search (the latter only when requested; `dhcp.domain`, else `dns.localDomain`), 28 broadcast address, 51 lease time, 54 server identifier (PiCache's address), 58/59 T1/T2 (50 % / 87.5 %), 12 host name (the lease's). 53 and 54 come first, then the options of the client's parameter request list in its order, then the rest. A reply never exceeds 576 bytes unless the client's option 57 allows more (up to 1500); options that do not fit are left out; replies are padded to 300 bytes. A NAK carries 53 and 54 only; an INFORM answer no lease times.
...
- **Client identification**: the MAC (`chaddr`); option 61 is stored for display.
- **Storage** (picache.db, component `dhcp`, own append-only migrations): `dhcp_static(mac TEXT PRIMARY KEY, ip TEXT NOT NULL UNIQUE, hostname, comment, created_at, updated_at)` (≤ 1024), `dhcp_leases(mac TEXT PRIMARY KEY, ip TEXT NOT NULL UNIQUE, hostname, client_id, expires_at, updated_at)` (≤ 4096; when full, the lease that expired first makes room). Timestamps are unix milliseconds. Leases are written on every change and survive restarts; expired leases are kept 24 h to hand the same address back and are purged every 10 minutes. Rows an edited database made invalid are skipped at load. Backups include both tables (nothing secret).
- **Host names**: option 12, else the first label of option 81, sanitised to a DNS label (lower-case `a-z0-9-`, space and underscore become `-`, other characters are dropped, at most 63, no leading or trailing `-`; empty → none); a renewal without a name keeps the previous one. A static lease's host name wins and is reserved for its client. A name held by another active lease is not used for the second one (no DNS name, `nameConflict` in the API); a lease keeps its name while it is active, free names go to active leases in MAC order.

### 18.5 IPv6 announcements

The router advertisements and stateless DHCPv6 need DHCP enabled and available, a usable interface and a **stable ULA** of PiCache on it (not temporary, not deprecated, not tentative; a statically configured one first; global addresses change with the prefix and are never announced), otherwise they are `blocked` (`no-interface`, `no-ula`, `no-raw-socket` or `no-socket`). The IPv4 blockers (`dynamic-address`, `other-server`, `range`) do not stop them. The router's own DNS announcement should be switched off where possible, otherwise devices may keep asking the router.
